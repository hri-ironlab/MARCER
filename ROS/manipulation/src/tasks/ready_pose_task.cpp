#include <tasks/ready_pose_task.h>

const bool registered = TaskFactory::registerTask(
    manipulation::ManipulationPlanRequest::READY_POSE,
    [](const std::string &taskName, const ros::NodeHandle &nh) -> std::unique_ptr<TaskBase>
    {
      return std::make_unique<ReadyPoseTask>(taskName, nh);
    });

ReadyPoseTask::ReadyPoseTask(const std::string &task_name, const ros::NodeHandle &nh) : TaskBase(task_name, nh)
{
  current_state_stage_ = nullptr;
}

bool ReadyPoseTask::init(const TaskParameters &parameters)
{
   ...
   ***************************************************/
  {
    ...
    applicability_filter->setPredicate([&](const SolutionBase &s, std::string &comment)
          ...
          {
            comment = "Object attached to hand, place it first";
            return false;
          }
          return true; });
  ...
  {
    auto stage = std::make_unique<stages::MoveTo>("ready pose", sampling_planner);
