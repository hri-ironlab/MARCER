...
const bool registered = TaskFactory::registerTask(
    ...
    {
      return std::make_unique<CloseHandTask>(taskName, nh);
    });

CloseHandTask::CloseHandTask(const std::string &task_name, const ros::NodeHandle &nh) : TaskBase(task_name, nh)
{
  current_state_stage_ = nullptr;
}

bool CloseHandTask::init(const TaskParameters &parameters)
{
