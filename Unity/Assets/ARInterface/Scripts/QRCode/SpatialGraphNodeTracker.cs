...
//https://github.com/yl-msft/QRTracking/blob/main/SampleQRCodes/Assets/Scripts/SpatialGraphNodeTracker.cs
// Updated main camera cache
        ...
        void Update()
        {
            ...
            if (node != null)
            {
                if (node.TryLocate(FrameTime.OnUpdate, out Pose pose))
                {
                    ...
                    if (Utils.MainCamera.transform.parent != null)
                    {
                        pose = pose.GetTransformedBy(Utils.MainCamera.transform.parent);
                    }
                    ...
                    setWorldOriginEvent.Raise(transform);
                }
                else
                {
